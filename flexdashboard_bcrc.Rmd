...
$(document).ready(function(){
  $('.navbar-author')
    .after('</br><span id=\"updatedat\">Updated: <span data-livestamp=\"", timestamp, "\"></span></span>');
});
</script>
", sep = "")
```
...
X <- X[, .(rank_range, img, price, asset_rank, asset_rarity, sc, market)]
X <- X[complete.cases(X)]
...
datatable(X, class = 'compact stripe hover',
          options = list(dom = "t",
                         ...
                         info = FALSE,
                         columnDefs = list(list(className = 'dt-right', targets = "_all"))),
          colnames = c("Rank range", "Asset", "Price", "Rank", "Rarity", "Sc", "Market"),
          rownames = FALSE,
          escape = FALSE) %>%
  formatStyle(columns = 1:ncol(X), fontSize = '90%')
```
