...
JPGS[, asset_number  := as.numeric(gsub("SpaceApe #", "", asset))]
...
# Database evolution -------------------------------------------------------------------------------
DTE <- copy(DT)
if (file.exists("data/DTE_sac.rds")) {
  cat("File data/DTE exists:", file.exists("data/DTE_sac.rds"), "\n")
  DTE_old <- readRDS("data/DTE_sac.rds")
  DTE <- rbindlist(list(DTE, DTE_old))
  DTE <- DTE[difftime(time_now, data_date, units = "hours") <= 24] # Only retain last 24 hours
}
saveRDS(DTE, file = "data/DTE_sac.rds")
